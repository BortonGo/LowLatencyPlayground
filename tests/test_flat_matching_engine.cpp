...
TEST(FlatMatchingEngine, AddBuyRestsWhenBookIsEmpty) {
    llp::FlatMatchingEngine engine(50, 150);
...
TEST(FlatMatchingEngine, AddSellRestsWhenBookIsEmpty) {
    llp::FlatMatchingEngine engine(50, 150);
    std::vector<llp::Trade> trades;
...
TEST(FlatMatchingEngine, BuyCrossesAskAndCreatesTrade) {
    llp::FlatMatchingEngine engine(50, 150);
    std::vector<llp::Trade> trades;
...
TEST(FlatMatchingEngine, SellCrossesBidAndCreatesTrade) {
    llp::FlatMatchingEngine engine(50, 150);
    std::vector<llp::Trade> trades;
...
TEST(FlatMatchingEngine, PartialFillIncomingBuy) {
    llp::FlatMatchingEngine engine(50, 150);
    std::vector<llp::Trade> trades;
...
TEST(FlatMatchingEngine, PartialFillRestingAsk) {
    llp::FlatMatchingEngine engine(50, 150);
    std::vector<llp::Trade> trades;
...
TEST(FlatMatchingEngine, MultipleLevelsMatchInPriceTimeOrder) {
    llp::FlatMatchingEngine engine(50, 150);
    std::vector<llp::Trade> trades;
...
TEST(FlatMatchingEngine, FIFOWithinSamePrice) {
    llp::FlatMatchingEngine engine(50, 150);
    std::vector<llp::Trade> trades;
...
TEST(FlatMatchingEngine, CancelRestingOrder) {
    llp::FlatMatchingEngine engine(50, 150);
    std::vector<llp::Trade> trades;
...
TEST(FlatMatchingEngine, CancelFilledOrderReturnsFalse) {
    llp::FlatMatchingEngine engine(50, 150);
    std::vector<llp::Trade> trades;
...
TEST(FlatMatchingEngine, NonCrossingOrderRests) {
    llp::FlatMatchingEngine engine(50, 150);
    std::vector<llp::Trade> trades;
...
TEST(FlatMatchingEngine, ReserveAndMakeTrades) {
    llp::FlatMatchingEngine engine(50, 150);
    engine.reserve(100);
    std::vector<llp::Trade> trades;
...
TEST(FlatMatchingEngine, TradeBuffer) {
    llp::FlatMatchingEngine engine(50, 150);
    std::vector<llp::Trade> trades(100);
    llp::TradeBuffer trade_buffer(trades.data(), trades.size());

    auto sell1 = make_order(llp::OrderSide::Sell, 1, 101, 3);
    EXPECT_TRUE(engine.add(sell1, trade_buffer));

    auto sell2 = make_order(llp::OrderSide::Sell, 2, 100, 4);
    EXPECT_TRUE(engine.add(sell2, trade_buffer));

    auto sell3 = make_order(llp::OrderSide::Sell, 3, 102, 5);
    EXPECT_TRUE(engine.add(sell3, trade_buffer));

    auto buy = make_order(llp::OrderSide::Buy, 10, 102, 12);
    EXPECT_TRUE(engine.add(buy, trade_buffer));

    ASSERT_EQ(trade_buffer.size(), 3);
    EXPECT_EQ(trades[0].buy_order_id, 10);
    EXPECT_EQ(trades[0].sell_order_id, 2);
    EXPECT_EQ(trades[0].price, 100);
    EXPECT_EQ(trades[0].quantity, 4);
